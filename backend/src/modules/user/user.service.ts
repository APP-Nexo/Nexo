...
export class UserService {
    static async searchUser(
        ...
        find?: string,
        cursor?: string,
    ): Promise<UsersListTotalResponse<UserPublicSearchResult>> {
        if (!find) return { users: [], nextCursor: null, total: 0 };
    ...
    static async getUser(tokenUser: UserPayload, id: number): Promise<UserResponseDTO> {
