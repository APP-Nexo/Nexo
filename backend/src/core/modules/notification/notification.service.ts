...
export class NotificationService {
	static async getNotifications(userId: number, cursor?: string) {
		...
		const nextCursor =
			notifications.length === 6 ? (notifications[5]?.id ?? null) : null;
	...
	static async readAllNotifications(userId: number) {
		await NotificationErrors.ensureHasUnreadNotifications(
			prisma.notification,
			userId,
		);
	...
	static async deleteAllNotifications(userId: number) {
		await NotificationErrors.ensureHasNotifications(
			prisma.notification,
			userId,
		);
