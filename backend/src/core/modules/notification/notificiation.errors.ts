export class NotificationErrors extends Error {
	...
	static async ensureNotificationExists(
		table: any,
		id: number,
		toUserId: number,
	) {
	...
	static async ensureHasNotifications(table: any, toUserId: number) {
		...
		if (count === 0)
			NotificationErrors.throw("Você não possui notificações.", 404);
	}
	...
	static async ensureHasUnreadNotifications(table: any, toUserId: number) {
		...
		if (count === 0)
			NotificationErrors.throw("Você não possui notificações não lidas.", 404);
	}
}
