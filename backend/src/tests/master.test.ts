...
describe('Master Routes', () => {
    describe('PATCH /user/:id/promote', () => {
        it('should promote a user to admin successfully', async () => {
            ...
            vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({
                id: 2, roleId: 1, role: { role: 'user' }
            } as any)

            // ensureUserExistById: user.findUnique
            vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({
                id: 2, name: 'User to Promote', email: 'user@email.com', roleId: 1
            } as any)
    ...
    describe('PATCH /user/:id/demote', () => {
        it('should demote a user to user role successfully', async () => {
            // ensureNotMaster
            vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({
                id: 2, roleId: 2, role: { role: 'user' }
            } as any)

            // ensureUserExistById
            vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({
                id: 2, name: 'Admin to Demote', email: 'admin@email.com', roleId: 2
            } as any)
    ...
    describe('PATCH /user/:id/ban', () => {
        it('should ban a user successfully', async () => {
            ...
            // ensureUserExistById
            vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({
                id: 2, name: 'User to Ban', email: 'banned@email.com', roleId: 1
            } as any)

            // user.findUnique para pegar email atual
            vi.mocked(prisma.user.findUnique).mockResolvedValueOnce({
                id: 2, email: 'banned@email.com', activate: true
            } as any)

            // user.update
            vi.mocked(prisma.user.update).mockResolvedValueOnce({
                id: 2, email: 'banned_2_banned@email.com', activate: false, deletedAt: new Date()
            } as any)
